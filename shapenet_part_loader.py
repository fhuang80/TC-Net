...
class PartDataset(data.Dataset):
    def __init__(self, root, npoints=2500, classification=False, class_choice=None, split='train', normalize=True, Predict=False):
...
#            point_set = np.loadtxt(fn[1]).astype(np.float32)
            point_set = np.loadtxt(fn[1]).astype(np.float64)
            if self.normalize:
                point_set, length_ratio, max_val, min_pc = self.pc_normalize(point_set)
        ...
        if self.classification and self.Predict:
            return point_set, cls, length_ratio, filename, max_val, min_pc
        elif self.classification:
            return point_set, cls, length_ratio
        else:
    ...
    def pc_normalize(self, pc):
        ...
        length_ratio = max_pc - min_pc
        max_val = np.max(length_ratio)
        length_ratio = length_ratio / max_val
        return pc, length_ratio, max_val, min_pc
